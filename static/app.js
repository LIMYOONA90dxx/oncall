...
class SuperBizAgentApp {
    ...
    initializeElements() {
        ...
        this.authTenantId = document.getElementById('authTenantId');
        ...
        // 评测相关元素
        this.evaluationBtn = document.getElementById('evaluationBtn');
        this.evaluationModal = document.getElementById('evaluationModal');
        this.evaluationForm = document.getElementById('evaluationForm');
        this.evalCloseBtn = document.getElementById('evalCloseBtn');
        this.evalQuestion = document.getElementById('evalQuestion');
        this.evalAnswer = document.getElementById('evalAnswer');
        this.evalContexts = document.getElementById('evalContexts');
        this.evalGroundTruth = document.getElementById('evalGroundTruth');
        this.evalError = document.getElementById('evalError');
        this.evalSubmitBtn = document.getElementById('evalSubmitBtn');
        this.evalResults = document.getElementById('evalResults');
        this.evalScores = document.getElementById('evalScores');
        this.evalMode = 'manual';
        this.manualEvalForm = document.getElementById('manualEvalForm');
        this.autoEvalForm = document.getElementById('autoEvalForm');
        this.autoEvalSamples = document.getElementById('autoEvalSamples');
        this.autoEvalSource = document.getElementById('autoEvalSource');
        this.kbStats = document.getElementById('kbStats');
        this.evalProgress = document.getElementById('evalProgress');
    ...
    bindEvents() {
        ...
        if (this.evaluationBtn) {
            this.evaluationBtn.addEventListener('click', () => this.showEvaluationModal());
        }
        
        if (this.evalCloseBtn) {
            this.evalCloseBtn.addEventListener('click', () => this.hideEvaluationModal());
        }
        ...
        if (this.evaluationForm) {
            this.evaluationForm.addEventListener('submit', (e) => this.handleEvaluationSubmit(e));
        }
        
        // 评测模式切换
        document.querySelectorAll('.eval-tab').forEach(tab => {
            tab.addEventListener('click', (e) => {
                this.switchEvalMode(e.target.dataset.mode);
            });
        });
    }
    ...
    async uploadFile(file) {
        ...
        try {
            ...
            // 获取认证 headers（但不包含 Content-Type，让浏览器自动设置）
            const headers = {};
            if (this.authToken) {
    ...
    toggleAuthMode() {
        this.isAuthModeLogin = !this.isAuthModeLogin;
        if (this.isAuthModeLogin) {
            this.authModalTitle.textContent = '登录';
            this.authSubmitBtn.textContent = '登录';
            this.authSwitchText.textContent = '还没有账号？';
            this.authSwitchLink.textContent = '注册';
            this.registerFields.style.display = 'none';
        } else {
            this.authModalTitle.textContent = '注册';
            this.authSubmitBtn.textContent = '注册';
            this.authSwitchText.textContent = '已有账号？';
            this.authSwitchLink.textContent = '登录';
    ...
    async handleAuthSubmit(e) {
        ...
        const tenantId = this.authTenantId.value.trim();
        ...
        this.authSubmitBtn.textContent = this.isAuthModeLogin ? '登录中...' : '注册中...';
        
        try {
            const endpoint = this.isAuthModeLogin ? '/auth/login' : '/auth/register';
            const body = this.isAuthModeLogin 
                ? { username, password }
                : { username, password, tenant_id: tenantId || undefined };
            ...
            if (response.ok && data.access_token) {
                ...
                this.hideAuthModal();
                this.updateAuthUI();
                this.showNotification(`${this.currentUser.username}，欢迎回来！`, 'success');
            } else {
                let errorMsg = '操作失败';
                if (data.detail) {
            ...
            this.authSubmitBtn.textContent = this.isAuthModeLogin ? '登录' : '注册';
        }
    }
    ...
    updateAuthUI() {
        if (this.currentUser) {
            ...
            if (this.userTenant) this.userTenant.textContent = `租户: ${this.currentUser.tenant_id}`;
        } else {
    ...
    showEvaluationModal() {
        ...
        if (this.evaluationModal) {
            ...
            this.evalError.style.display = 'none';
            this.switchEvalMode('manual');
    ...
    switchEvalMode(mode) {
        ...
        if (this.manualEvalForm) {
            this.manualEvalForm.style.display = mode === 'manual' ? 'block' : 'none';
        }
        if (this.autoEvalForm) {
            this.autoEvalForm.style.display = mode === 'auto' ? 'block' : 'none';
        }
    }
    ...
    async loadKnowledgeBaseStats() {
        try {
            ...
            if (response.ok && data.code === 200) {
                if (this.kbStats) {
                    this.kbStats.innerHTML = `知识库文档数: <strong>${data.data.total_documents}</strong> 条`;
                }
            }
        } catch (error) {
    ...
    async handleEvaluationSubmit(e) {
        ...
        const metrics = Array.from(metricsChecked).map(cb => cb.value);
        
        if (metrics.length === 0) {
            this.showEvalError('请至少选择一个评测指标');
            return;
        }
        
        this.evalSubmitBtn.disabled = true;
        this.evalSubmitBtn.textContent = '评测中...';
        this.evalError.style.display = 'none';
        this.evalResults.style.display = 'none';
        
        // 显示进度条
        this.showProgress(0, '准备中...');
        ...
        try {
            ...
            if (this.evalMode === 'auto') {
                this.showProgress(10, '正在从知识库获取文档...');
                const numSamples = parseInt(this.autoEvalSamples.value) || 5;
                const sourceFilter = this.autoEvalSource.value.trim() || null;
                
                this.showProgress(20, '正在生成问答对...');
                
                response = await fetch(`${this.apiBaseUrl}/auto`, {
                    method: 'POST',
                    headers: this.getAuthHeaders(),
                    body: JSON.stringify({
                ...
                this.showProgress(30, '正在评测...');
                const question = this.evalQuestion.value.trim();
                const answer = this.evalAnswer.value.trim();
                const contextsText = this.evalContexts.value.trim();
                const groundTruth = this.evalGroundTruth.value.trim();
                
                if (!question || !answer) {
                    this.showEvalError('问题和答案不能为空');
                    this.evalSubmitBtn.disabled = false;
                    this.evalSubmitBtn.textContent = '开始评测';
                    this.hideProgress();
                    return;
                }
                ...
                response = await fetch(`${this.apiBaseUrl}/evaluate`, {
                    method: 'POST',
                    headers: this.getAuthHeaders(),
                    body: JSON.stringify({
                        samples: [sample],
            ...
            this.showProgress(80, '正在计算评测指标...');
            
            const data = await response.json();
            
            this.showProgress(100, '评测完成！');
            
            if (response.ok && data.code === 200) {
                this.displayEvaluationResults(data);
            } else {
                this.showEvalError(data.detail || data.message || '评测失败');
            }
        } catch (error) {
            console.error('评测失败:', error);
            this.showEvalError('网络错误，请稍后重试');
        } finally {
    ...
    showProgress(percent, text) {
        if (this.evalProgress) {
            this.evalProgress.style.display = 'block';
        }
    ...
    hideProgress() {
        if (this.evalProgress) {
            this.evalProgress.style.display = 'none';
        }
    }
    
    showEvalError(message) {
        if (this.evalError) {
            this.evalError.textContent = message;
            this.evalError.style.display = 'block';
        }
    }
    
    displayEvaluationResults(data) {
        if (!this.evalScores || !this.evalResults) return;
        
        const result = data.detailed_results && data.detailed_results[0];
        if (!result) return;
        
        const metricsInfo = {
            faithfulness: { name: '忠实度', desc: '答案是否基于上下文' },
            context_precision: { name: '上下文精确度', desc: '检索的上下文是否精确' },
            context_recall: { name: '上下文召回率', desc: '检索的上下文是否完整' }
        };
            ...
            if (value !== null && value !== undefined) {
                ...
                html += `
                    <div class="eval-score-item">
                        <div>
                            <div class="eval-score-name">${info.name}</div>
                            <div style="font-size: 12px; color: #5f6368;">${info.desc}</div>
                            <div class="eval-score-bar">
                                <div class="eval-score-fill" style="width: ${percentage}%"></div>
                            </div>
                        </div>
                        <div class="eval-score-value">${percentage}%</div>
                    </div>
                `;
            }
        }
        
        this.evalScores.innerHTML = html;
